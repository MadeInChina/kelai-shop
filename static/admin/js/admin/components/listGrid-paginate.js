    ...
    BLCAdmin.listGrid.paginate = {
        ...
        loadRecords : function($tbody, baseUrl) {
            ...
            if (startIndex != null && maxIndex != null) {
                ...
                BLCAdmin.listGrid.paginate.releaseLock();
                var startIndexParam = getUrlParam('startIndex');
                var totalrecords = parseInt($('#listGrid-main').find('tbody').data('totalrecords'));
                var recordranges = $('#listGrid-main').find('tbody').data('recordranges');
                if (recordranges) {
                    var arr = recordranges.split('-');
                    if (startIndexParam && totalrecords == parseInt(arr[1]) + 1) {
                        var firstLoadRecordsFlag = $.cookie('FIRST_LOAD_RECORDS');
                        if (!firstLoadRecordsFlag) {
                            var blankPadding = $($('.blank-padding')[0]);

                            var topVisibleIndex = parseInt(arr[0]);
                            var rowHeight = this.getRowHeight($tbody);
                            blankPadding.css({
                                height: Math.ceil(topVisibleIndex * rowHeight) + 'px'
                            });
                            ...
                            /*   $($('.mCustomScrollBox')[0]).css({
                                   'height' : '100%',
                                   'max-height' : '617px'
                               });*/
                            //var topIndex = parseInt(startIndexParam);
                            this.scrollToIndex($tbody, parseInt(startIndexParam));

                            var mCustomScrollBoxH = $($('.mCustomScrollBox')[0]).height();

                            var mCSB_containerH = $($('.mCSB_container')[0]).outerHeight();

                            /* $($('.listgrid-body-wrapper')[0]).css({
                                 'max-height' : $($('.mCustomScrollBox')[0]).height() + 'px'
                             });*/

                            var mCSB_draggerContainerH = $($('.mCSB_draggerContainer')[0]).height();

                            var draggerH = Math.round(mCustomScrollBoxH / mCSB_containerH * mCSB_draggerContainerH);

                            $($('.mCSB_dragger')[0]).css({
                                'height': draggerH + 'px',
                                'top': (mCustomScrollBoxH - 2 - draggerH) + 'px'
                            });

                            $($('.mCSB_dragger_bar')[0]).css({
                                "line-height": draggerH + 'px'
                            });
                            //$($('.listgrid-body-wrapper')[0]).mCustomScrollbar('update');
                            $($('.listgrid-body-wrapper')[0]).mCustomScrollbar("scrollTo", $($('.mCSB_container')[0]).position().top, {
                                trigger: "internal",
            ...
            $.cookie('FIRST_LOAD_RECORDS','LOADED');
        },
