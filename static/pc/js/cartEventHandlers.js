...
$body.on('click', '#cart-checkout', function() {
    var orderItemsRule = $('.orderItemShippingPricingRule');
    var arr = [];
    for (var i=0;i<orderItemsRule.length;i++){
        var tmp = $(orderItemsRule[i]).val().split(':');
        arr.push({
            orderItemId : tmp[0],
            shippingPricingRuleId : tmp[1]
        });
    }
    $.ajax({
        url :'/cart/saveOrderItemsShippingPricingRule?csrfToken=' + params.csrfToken,
        type : 'POST',
...
$('.orderItemShippingPricingRule').change(function () {
    if($(this).val() != ''){
        var tmp = $(this).val().split(':');
        $.ajax({
            url :'/cart/saveOrderItemShippingPricingRule?csrfToken=' + params.csrfToken,
            type : 'POST',
            data : JSON.stringify({
                orderItemId : tmp[0],
                shippingPricingRuleId : tmp[1]
