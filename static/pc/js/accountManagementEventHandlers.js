...
$body.on('click', '.js-manage-address', function() {
    ...
    $('.js-manage-address').removeClass('active');
        ...
        $formInfo.find('input:not(.btn, :radio,[name=csrfToken],[name=address\\.isoCountryAlpha2]), select:not(.js-chooseAddress)')
            .each(function(i, el) {
                ...
                if ($(el).is('select')) {
                    ...
                    /*$(el).empty();
                    $(el).append($($(data).find("select[name='" + $(el).attr('name') + "']")[0]).clone(true));*/
                    $(el).empty();
                    console.log('name = ' + $(el).attr('name'));
                    $(data).find("select[name='" + $(el).attr('name') + "'] option").each(function () {
                        var that = $(this);
                        console.log(that.text() + ':' + that.val());
                        $(el).append('<option value="' + that.val() + '">' + that.text() +'</option>');
                    });
                } else {
...
var showQrcode =function(orderId){
    $('#qrcodeModal').modal('show');
    //$('#loading').show();
    $('#qrcode').attr('src','/account/orders/qrcode/' + orderId);
    $('#barcode').attr('src','/account/orders/barcode/' + orderId);
    $.ajax({
       url :'/account/orders/writeOffCode/' + orderId,
       type:'GET',
       success:function (response) {
           $('#writeOffCode').html(response);
       }
    });
};
