...
public class ChangePasswordController extends BroadleafChangePasswordController {
    ...
    public String processChangePassword(HttpServletRequest request, Model model, @ModelAttribute("changePasswordForm") ChangePasswordForm form, BindingResult result, RedirectAttributes redirectAttributes) throws ServiceException {
        ...
        this.changePasswordValidator.validate(passwordChange, result);
        if (result.hasErrors()) {
            return this.getChangePasswordView();
        } else {
            this.customerService.changePassword(passwordChange);
            redirectAttributes.addAttribute("successMessage","修改密码成功");
    ...
    public String getChangePasswordRedirect() {
        return super.getChangePasswordRedirect();
    }
}
