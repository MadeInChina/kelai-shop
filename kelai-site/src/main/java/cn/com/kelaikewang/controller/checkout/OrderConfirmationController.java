...
public class OrderConfirmationController extends BroadleafOrderConfirmationController {
    ...
    public String displayOrderConfirmationByOrderNumber(@PathVariable("orderNumber") String orderNumber, Model model,
        ...
        String view = super.displayOrderConfirmationByOrderNumber(orderNumber, model, request, response);
        Order order = (Order)model.asMap().get("order");
        List<OrderPayment> orderPayments = order.getPayments();
        String gatewayType = orderPayments.get(0).getGatewayType().getType();
