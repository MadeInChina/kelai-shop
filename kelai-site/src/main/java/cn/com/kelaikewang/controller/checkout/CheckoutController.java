...
import org.broadleafcommerce.core.web.expression.checkout.CheckoutFormVariableExpression;
...
public class CheckoutController extends BroadleafCheckoutController {
    ...
    @Resource(name = "blCheckoutFormVariableExpression")
    protected CheckoutFormVariableExpression checkoutFormVariableExpression;
    @Resource(name = "blDefaultCurrentPaymentRequestService")
    ...
    protected String getActiveCheckoutStage(HttpServletRequest request) {
        ...
        if (activeStage == null) {
            if (!cartStateService.cartHasPopulatedShippingAddress() && checkoutFormVariableExpression.shouldShowShippingInfoStage()) {
                ...
                activeStage = ZaoJiCMSCheckoutStageType.SUBMIT_ORDER.getType();
    ...
    @RequestMapping(value = "/checkout/submitOrder",method = RequestMethod.POST)
    @ResponseBody
    public ResponseDTO submitOrder(){
        Order order = CartState.getCart();
        if (!OrderStatus.IN_PROCESS.getType().equals(order.getStatus().getType())){
            return ResponseDTO.fail("非处理中的订单无法提交");
        }
        orderService.confirmOrder(order);
        return ResponseDTO.success("订单已提交成功",order.getId());
    }
    ...
    public String payment(@RequestParam("orderId")long orderId,Model model){
        PaymentRequestDTO paymentRequest = currentOrderPaymentRequestService.getPaymentRequestFromCurrentOrder();
